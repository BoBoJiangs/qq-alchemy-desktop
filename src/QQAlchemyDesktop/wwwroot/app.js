...
$('saveSettingsBtn').addEventListener('click', () => busy($('saveSettingsBtn'), async () => { const s = { ...(currentSettings?.alchemy || {}), danNumber:Number($('danNumber').value), alchemy:$('alchemyMode').value === 'true', alchemyNumber:Number($('alchemyNumber').value), makeNumber:Number($('makeNumber').value), taskPurchaseLimit:Number($('taskPurchaseLimit').value), emptyMarketRoundsBeforeStop:Number($('emptyRounds').value), limitHerbsCount:Number($('limitHerbsCount').value), randomDelay:Number($('randomDelay').value), dryRun:$('dryRunSetting').checked }; await api('/api/settings/alchemy', { method:'PUT', body:JSON.stringify(s) }); currentSettings.alchemy = s; toast('安全设置已保存'); await refreshStatus(); }));
