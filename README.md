...
- Windows UI Automation 定位 QQ 主窗口、群标题、输入框和 @ 建议；窗口捕获使用 `Windows.Graphics.Capture`，中文识别使用 `Windows.Media.Ocr`。
- 校准保存 QQ 版本、窗口尺寸、DPI 及相对区域。每次发送前重新核验指纹和群标题。
- @ 操作必须同时确认显示名和 QQ 号 `3889001741`；无法确认时不会发送。
- OCR 需要连续两次规范化结果一致；坊市商品保存帧哈希、页码、矩形和动作编号，重新截图不一致时拒绝点击。
...
测试覆盖配方阈值、性平药引、收益排序、手续费、背包解析、唯一 OCR 纠错、坊市单位换算、重复采购优先、普通价格回退、库存上限、任务购买上限、响应区域去重和 Windows.Graphics.Capture/OCR 集成。若开发机存在当前 Java 账号数据，还会比较 Java/C# 规范化配方集合；当前账号 `3860863656` 已通过完整集合比对。
