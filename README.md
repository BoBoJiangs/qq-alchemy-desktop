...
- @ 操作默认同时确认显示名和 QQ 号 `3889001741`；无法确认时不会发送。任务安全设置可开启“不 @ 机器人也允许触发命令”，兼容无需 @ 的机器人配置。
- 静态区域 OCR 需要连续两次规范化结果一致；聊天区域遇到 QQ 滚动刷新时会追加第三帧确认，PP-OCRv5 输出文字、置信度和行坐标，并使用 QQ 蓝色像素收紧药材链接的真实点击区域。静态冲突和聊天刷新都会写入审计记录。
- 坊市采购采用“两阶段”流程：先对当前页所有符合规则的药材逐个点击，只校验并保存输入框生成的 `坊市购买<UUID>`，全部采集完成后再按页面顺序快速串行发送；采集期间不发送命令，因此不会因购买结果滚动卡片，也不会为每件药材重新查询坊市。采购码优先走 UIA 快速读取，购买结果也优先走 UIA 并以 OCR 回退，页面在采集阶段发生变化或命令校验失败时安全暂停。
